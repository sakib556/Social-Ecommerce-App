...
class OfferSentPage extends StatefulWidget {
  const OfferSentPage({Key? key}) : super(key: key);
  @override
  State<OfferSentPage> createState() => _OfferSentPageState();
}

class _OfferSentPageState extends State<OfferSentPage> {
  int taskIndex = 0;
  int widgetIndex = 0;
...
class AllOffer extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return UserOrdersBuilder(builder: ((context, userOffersList, orderList) {
      final sentAllOffers = userOffersList
          .where((offer) =>
              (!offer.isPostOffer &&
          ...
          .toList();
      final sentToExploreOffers = userOffersList
          .where((offer) =>
              !offer.isPostOffer &&
              offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
      final sentToPostOffers = userOffersList
          .where((offer) =>
              offer.isPostOffer &&
              offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
      final list = [sentAllOffers, sentToPostOffers, sentToExploreOffers];
              ...
              children: list[placeIndex].map((userOffer) {
                final userId =
                    userOffer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid
                        ? userOffer.sellerId
                        : userOffer.buyerId;
                return OfferCard(offerModel: userOffer, fromReceive: false, fromSent: true,);
              }).toList())
          : const Center(
              child: Text("No offers sent."),
            );
    }));
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return UserOrdersBuilder(builder: ((context, userOffersList, orderList) {
      final pendingOffers = userOffersList
          ...
          .toList();
      final sentAllOffers = pendingOffers
          .where((offer) =>
              (!offer.isPostOffer &&
                  offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid) ||
              (offer.isPostOffer &&
                  offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid))
          .toList();
      final sentToExploreOffers = pendingOffers
          .where((offer) =>
              !offer.isPostOffer &&
              offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
      final sentToPostOffers = pendingOffers
          .where((offer) =>
              offer.isPostOffer &&
              offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
              ...
              children: list[placeIndex].map((userOffer) {
                final userId =
                    userOffer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid
                        ...
                        : userOffer.buyerId;
                return OfferCard(
                        offerModel: userOffer, fromReceive: false, fromSent: true,
                      );
              }).toList())
          : const Center(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return UserOrdersBuilder(builder: ((context, userOffersList, orderList) {
      final acceptOffers = userOffersList
          .where((offer) => offer.offerAccepted && !offer.offerCancelled)
          .toList();
      final sentAllOffers = acceptOffers
          .where((offer) =>
              (!offer.isPostOffer &&
                  offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid) ||
              (offer.isPostOffer &&
                  offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid))
          .toList();
      final sentToExploreOffers = acceptOffers
          .where((offer) =>
              !offer.isPostOffer &&
              offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
      final sentToPostOffers = acceptOffers
          .where((offer) =>
              offer.isPostOffer &&
              offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return UserOrdersBuilder(builder: ((context, userOffersList, orderList) {
      final cancelledOffers = userOffersList
          .where((offer) => !offer.offerAccepted && offer.offerCancelled)
          .toList();
      final sentAllOffers = cancelledOffers
          .where((offer) =>
              (!offer.isPostOffer &&
                  offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid) ||
              (offer.isPostOffer &&
                  offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid))
          .toList();
      final sentToExploreOffers = cancelledOffers
          .where((offer) =>
              !offer.isPostOffer &&
              offer.buyerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
      final sentToPostOffers = cancelledOffers
          .where((offer) =>
              offer.isPostOffer &&
              offer.sellerId == FirebaseConstants.firebaseInstance.currentUser!.uid)
          .toList();
