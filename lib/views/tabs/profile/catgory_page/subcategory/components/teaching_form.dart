...
class TeachingForm extends ConsumerWidget {
  const TeachingForm({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return TeachingBuilder(builder: ((context, userServices) {
      bool forRent = false;
      bool forSell = false;
      bool isCostPerHour = false;
      bool isCostPerDay = true;
      int _index = 1;
      final userService = ref.watch(teachingItemProvider);
      ...
      return Column(
        children: [
          ...
          CustomButton(
              text: "Add listing",
              onPressed: () async {
                EasyLoading.show(status: "Uploading your skills...");
                ...
                Future<void> _createPost(
                  ...
                  print("read start");
                  await ref
                      .read(teachingProvider.notifier)
                      .addTeachingListing(_serviceModel, context);
                  print("read done fe");
                }
                ...
                print("test last");
                EasyLoading.showSuccess("Your skills added.");
...
class ServiceTypeWidget extends ConsumerWidget {
  const ServiceTypeWidget({Key? key, required this.serviceTypeModel})
      : super(key: key);
  final TeachingInformationModel serviceTypeModel;
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      children: [
        CheckboxListTile(
          ...
          onChanged: (_) {
            ref.read(teachingItemProvider.notifier).toggle(serviceTypeModel);
          },
